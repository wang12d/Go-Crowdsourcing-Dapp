go: Task.abi Task.bin Platform.abi Platform.bin accounts.json
	...
	(ganache-cli -m "enable laugh there club flower source fog attract giraffe trend light rival" -a 60 --account_keys_path=$(shell pwd)/pkg/crowdsourcing/platform/accounts.json --verbose &)
